    ...
    <div class="mx-3 h-100 right-content">
        ...
        <div class="text-center mb-4 d-flex justify-content-center flex-wrap">
            ...
            <div class="nav d-flex justify-content-center w-100">
                <a href="{{ route('calendar', ['date' => $selectedDate->copy()->firstOfMonth()->subMonth()->format('Y-m-d')]) }}" type="button" class="btn btn-primary btn-sm month-nav prev-month me-4">
                ...
                <a href="{{ route('calendar', ['date' => $selectedDate->copy()->firstOfMonth()->addMonth()->format('Y-m-d')]) }}" type="button" class="btn btn-primary btn-sm month-nav next-month">
                    Next Month
                        ...
                        @for ($col = 0; $col < 7; $col++)
                            ...
                            <td data-date="{{ $date ? $date->format('d-m-Y') : '' }}"
                                ...
                                <div class="cell-wrapper h-100">
                                    <div class="cell-top">
                                        <span class="row-week">{{ $col == 0 && $date ? 'Week ' . $date->weekOfYear : '' }}</span>
                                    ...
                                    <div class="cell-content d-flex flex-column justify-content-between">
                                        @if($practiceClass)
                                            @for($session = 1; $session <= 3; $session++)
                                                @php
                                                    $isRegistered = $practiceClass->session == $session;
                                                @endphp
                                                <div data-date="{{ $date ? $date->format('d-m-Y') : '' }}" data-slot="{{ $session }}" class="cell-item{{ $isRegistered ? ' registered' : '' }}">
                                                    <div class="slot-labels">
                                                        <div class="slot-label">{{ $session == 1 ? 'S' : ($session == 2 ? 'C' : 'T') }}</div>
                                                    </div>
                                                    @if($isRegistered)
                                                        <div class="cell-class-details text-start h-100 p-2 d-flex flex-column justify-content-between">
                                                            {{--                                                            <p class="mb-0">{{$practiceClass->ten_lop_thuc_hanh}}</p>--}}
                                                            {{--                                                            <p class="mb-0">{{$practiceClass->teacher->ten_giang_vien}}</p>--}}
                                                            {{--                                                            <p class="mb-0">{{$practiceClass->ten_lop_thuc_hanh}}</p>--}}
                                                            <p class="mb-0">Quan ly cac du an Cong nghe thong tin asd as dsa dsad asdas dsad asd asd asd asd asd asd</p>
